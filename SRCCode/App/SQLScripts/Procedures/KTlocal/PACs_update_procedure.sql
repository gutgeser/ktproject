...
BEGIN
  ...
  SELECT 'Running PACs_update_procedure';
  ...
  PAC_loop:REPEAT
    ...
    SELECT COUNT(sample_id) INTO l_lookup_count FROM PACs WHERE sample_id = l_sampleid_int LIMIT 1; #what happens if there is NULL

    IF (l_lookup_count < 1) && (l_sampleid_int IS NOT NULL) THEN
