...
BEGIN
  ...
  OPEN cu2;
  animal_loop:REPEAT
    FETCH cu2 INTO l_SampleID, l_ClientID, l_Species, l_loc_lbl, l_Sex, l_col_year, l_col_month; #we declared the cursor, we are taking the info from the cursor and putting it into these declared variables

    SELECT id INTO l_locationid
    FROM locations WHERE lbl = l_loc_lbl LIMIT 1;
  ...
  OPEN cu3;
  spatial_loop:REPEAT
      FETCH cu3 INTO l_SampleID, l_Species, l_loc_lbl, l_Sex; #we declared the cursor, we are taking the info from the cursor and putting it into these declared variables

      SELECT id INTO l_locationid
      FROM locations WHERE lbl = l_loc_lbl LIMIT 1;

      SELECT COUNT(*) INTO sample_count
      FROM samples WHERE l_SampleID = sample_id;
