file=$_SERVER_PROJECT_ROOT_DIR'/SRCCode/MacData/PYScripts/'$_CONFIG_ACTIONPARAM
if [[ -f $file ]]; then
  python3 'SRCCode/MacData/PYScripts/'$_CONFIG_ACTIONPARAM $_MYSQL_USER $_MYSQL_USER_PASSWORD $_MYSQL_HOST_IP $_CONFIG_DATABASE
else
  echo 'No script at '$_SERVER_PROJECT_ROOT_DIR'/SRCCode/MacData/PYScripts/'$_CONFIG_ACTIONPARAM' found'
fi
